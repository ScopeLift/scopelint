// Parses solidity comments. This module is based on the [`forgefmt` comments module](https://github.com/foundry-rs/foundry/blob/675a824648b6259c1566009b79356a4fc109b6ac/crates/fmt/src/comments.rs).
...
impl Comments {
    ...
    /// results of parsing comments which start with `scopelint:`
    ...
    ) -> impl Iterator<Item = Result<(Loc, InlineConfigItem), (Loc, InvalidInlineConfigItem)>> + '_
    {
        self.iter()
            .filter_map(|comment| {
                Some((comment, comment.contents().trim_start().strip_prefix("scopelint:")?.trim()))
            })
            .map(|(comment, item)| {
                let loc = comment.loc;
